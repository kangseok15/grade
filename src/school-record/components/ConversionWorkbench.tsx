    ...
    <div className="space-y-6" id="conversion-workbench-section">
      ...
      <div className="bg-white rounded-xl border border-stone-200 shadow-xs p-5">
        <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-stone-100 gap-4">
          ...
          {/* Model Switcher */}
          <div className="flex flex-wrap items-center gap-1.5 bg-stone-100 p-1 rounded-lg border border-stone-200 text-xs self-start md:self-auto shrink-0">
            <span className="text-stone-500 font-bold ml-1 mr-1">전체 적용 기준:</span>
            <button
              type="button"
              onClick={() => {
        ...
        <div className="pt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-xl bg-stone-50 border border-stone-200 flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-stone-700">대입 지원 가이드</span>
              <span className="text-[10px] px-1.5 py-0.5 rounded-sm bg-stone-200 text-stone-800 font-semibold">
                진학 진단
              </span>
            </div>
            ...
            <div className="text-[11px] text-stone-500 mt-1 pt-1 border-t border-stone-200">
              현재 <strong className="text-stone-700">{currentOfficeName}</strong> 공식 기준표가 전체 성적표에 적용 중입니다.
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto max-h-[480px] overflow-y-auto">
          {selectedTable === 'busan' && (
            <table className="w-full text-xs text-center border-collapse whitespace-nowrap min-w-[680px]">
              <thead className="bg-stone-100 text-stone-700 font-bold sticky top-0 z-10 border-b border-stone-200 whitespace-nowrap">
                <tr>
                  ...
                  <th className="py-2.5 px-3 text-stone-500 whitespace-nowrap">현재 학생 위치 대조</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-stone-200 text-stone-800 whitespace-nowrap">
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-stone-50 transition-colors whitespace-nowrap ${
                        isCurrent ? 'bg-amber-100/70 font-extrabold text-amber-950' : idx % 2 === 1 ? 'bg-stone-50/40' : ''
                      }`}
                    >
                      <td className="py-2 px-3 font-semibold text-blue-900 whitespace-nowrap">
                      ...
                      <td className="py-2 px-3 whitespace-nowrap">
                        {isCurrent ? (
                          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-900 bg-amber-200 px-2 py-0.5 rounded-full whitespace-nowrap">
                ...
                {GYEONGGI_CONVERSION_TABLE.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 px-3 font-mono text-stone-600 whitespace-nowrap">
                        {row.grade5Sum12}
                      </td>
                ...
                {GWANGJU_OFFICE_CONVERSION_TABLE.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 px-3 font-semibold text-stone-600 whitespace-nowrap">
            ...
            <table className="w-full text-xs text-center border-collapse whitespace-nowrap min-w-[850px]">
              <thead className="bg-stone-100 text-stone-700 font-bold sticky top-0 z-10 border-b border-stone-200 whitespace-nowrap">
                <tr>
                  ...
                  <th className="py-2.5 px-3 whitespace-nowrap text-stone-600">부산 (1.6만명) 9등급</th>
                  <th className="py-2.5 px-3 whitespace-nowrap text-stone-600">경기 (12과목합) 9등급</th>
                  <th className="py-2.5 px-3 whitespace-nowrap text-stone-600">광주 (고1과정) 9등급</th>
                  <th className="py-2.5 px-3 whitespace-nowrap bg-blue-50 text-blue-900 border-x border-blue-200">
                    ★ 3개 교육청 통합 평균 9등급
                  </th>
                  <th className="py-2.5 px-3 whitespace-nowrap bg-blue-50 text-blue-900 border-r border-blue-200">
                    평균 누적비 (%)
                  </th>
                  <th className="py-2.5 px-3 text-stone-500 whitespace-nowrap">현재 학생 위치 대조</th>
                </tr>
                ...
                {AVERAGE_OFFICE_CONVERSION_TABLE.map((row, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-stone-50 transition-colors whitespace-nowrap ${
                        isCurrent
                          ? 'bg-amber-100/70 font-extrabold text-amber-950'
                          : idx % 2 === 1
                          ? 'bg-stone-50/40'
                          : ''
                      }`}
                    >
                      ...
                      <td className="py-2 px-3 text-stone-600 whitespace-nowrap">
                        ...
                        <span className="text-[10px] text-stone-400 ml-1">({row.busanCumRatio.toFixed(1)}%)</span>
                      </td>
                      <td className="py-2 px-3 text-stone-600 whitespace-nowrap">
                        {row.gyeonggiGrade9.toFixed(2)}등급
                        <span className="text-[10px] text-stone-400 ml-1">({row.gyeonggiCumRatio.toFixed(1)}%)</span>
                      </td>
                      <td className="py-2 px-3 text-stone-600 whitespace-nowrap">
                        {row.gwangjuGrade9.toFixed(2)}등급
                        <span className="text-[10px] text-stone-400 ml-1">({row.gwangjuCumRatio.toFixed(1)}%)</span>
                      </td>
                      <td className="py-2 px-3 font-black text-blue-700 bg-blue-50/60 border-x border-blue-100 whitespace-nowrap">
                        {row.avgGrade9.toFixed(2)}등급
                      </td>
                      <td className="py-2 px-3 font-mono font-bold text-blue-900 bg-blue-50/60 border-r border-blue-100 whitespace-nowrap">
